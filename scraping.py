# Import Splinter, BeautifulSoup, Datetime, Webdriver and Pandas
...
def hemispheres(browser):
    ...
    for x in range(len(images)):
        ...
        # Find elements to click on
        browser.find_by_css("a.product-item h3")[x].click()

        # locating the "Sample" image link
        sample_image= browser.links.find_by_text('Sample').first

        hemispheres["img_url"] = sample_image['href']

        # Getting the Hemisphere titles
        hemispheres["title"]=browser.find_by_css("h2.title").text 

        # Adding urls to the hemisphere_image_urls list
